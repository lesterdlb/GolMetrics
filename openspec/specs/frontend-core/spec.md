...
- **THEN** it SHALL install: `react`, `react-dom`, `react-router-dom`, `axios`, `zustand`, `tailwindcss`, `@tailwindcss/vite`, `clsx`, `tailwind-merge`, `react-markdown`, `lucide-react`
...
- **THEN** it SHALL follow this structure under `src/GolMetrics.Web/src/`: `pages/` (route-level components), `components/` (reusable UI components), `services/` (API service modules), `store/` (Zustand stores), `types/` (TypeScript type definitions), `lib/` (utility functions like `cn()`)
